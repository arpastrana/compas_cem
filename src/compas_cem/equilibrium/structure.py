...
class EquilibriumStructure:
    ...
    def node_index(self):
        ...
        if not self._node_index:
            self._node_index = self.topology.key_index()
    ...
    def edge_index(self):
        ...
        if not self._edge_index:
            self._edge_index = self.topology.uv_index()
        return self._edge_index

    @property
    def connectivity(self):
        """
        The connectivity of the network encoded as a branch-node as a matrix.

        Rows correspond to edges and columns to nodes.
        """
        if self._connectivity is None:
            node_idx = self.node_index
            edges = [(node_idx[u], node_idx[v]) for u, v in self.topology.edges()]
