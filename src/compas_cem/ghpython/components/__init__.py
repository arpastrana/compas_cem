...
def install():
    ...
    try:
        ...
        results.append(('compas_cem', 'Installed {} GH User Objects'.format(len(userobjects)), True))
...
def uninstall():
    ...
    try:
        ...
        for f in _remove_old_ghpy_components(grasshopper_library_path):
            results.append(('compas_cem', 'Uninstalled old component: {}'.format(f), True))
        ...
        results.append(('compas_cem', 'Uninstalled {} GH User Objects'.format(len(userobjects)), True))
    except PermissionError:
        raise Exception('Please close first all instances of Rhino and then rerun the command')
...
def _remove_old_ghpy_components(grasshopper_library_path):
    oldlibs = glob.glob('%s*.ghpy' % os.path.join(grasshopper_library_path, 'COMPAS CEM_0.1'))
    for f in oldlibs:
        os.remove(f)
        yield f
